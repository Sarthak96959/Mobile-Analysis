for number in range(1,43):
    print("File Pages Number:",number)
...
# I want to join GB and MB after RAM and ROM 
    GB_lst = []
    for i in mob_name:
        if "GB" in i : 
            GB_lst.append("GB")
    ...
    GB_lst_ = pd.DataFrame({"GB/MB":GB_lst})
    table = pd.concat([mob_name_,brand_,mob_var_,mob_price_,mob_star_rating_,ratings_,mob_review1_,ram1,GB_lst_,rom1,GB_lst_,display_,battery_,camera_],axis = 1)

    print(GB_lst_.head(2))
    print()

    table.to_csv(f"a{number}.csv",index = False)
